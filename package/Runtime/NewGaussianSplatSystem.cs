...
namespace GaussianSplatting.Runtime
{
    class NewGaussianSplatRenderSystem
    {
        ...
        public Material SortAndRenderSplats(Camera cam, CommandBuffer cmb)
        {
            ...
            if (m_ActiveSplats.Count <= 0) return matComposite;
            ...
            // {
                ...
                {
                    NewGaussianSplatRenderer.RenderMode.DebugPoints => gs.m_MatDebugPoints,
                    NewGaussianSplatRenderer.RenderMode.DebugPointIndices => gs.m_MatDebugPoints,
                    NewGaussianSplatRenderer.RenderMode.DebugBoxes => gs.m_MatDebugBoxes,
                    NewGaussianSplatRenderer.RenderMode.DebugChunkBounds => gs.m_MatDebugBoxes,
                    _ => gs.m_MatSplats
                };
                if (displayMat == null)
                    return matComposite;
                ...
                mpb.SetBuffer(NewGaussianSplatRenderer.Props.SplatChunks, gs.m_GpuChunks);

                mpb.SetBuffer(NewGaussianSplatRenderer.Props.SplatViewData, gs.m_GpuView);

                mpb.SetBuffer(NewGaussianSplatRenderer.Props.OrderBuffer, gs.m_GpuSortKeys);
                mpb.SetFloat(NewGaussianSplatRenderer.Props.SplatScale, gs.m_SplatScale);
                mpb.SetFloat(NewGaussianSplatRenderer.Props.SplatOpacityScale, gs.m_OpacityScale);
                mpb.SetFloat(NewGaussianSplatRenderer.Props.SplatSize, gs.m_PointDisplaySize);
                mpb.SetInteger(NewGaussianSplatRenderer.Props.SHOrder, gs.m_SHOrder);
                mpb.SetInteger(NewGaussianSplatRenderer.Props.SHOnly, gs.m_SHOnly ? 1 : 0);
                mpb.SetInteger(NewGaussianSplatRenderer.Props.DisplayIndex, gs.m_RenderMode == NewGaussianSplatRenderer.RenderMode.DebugPointIndices ? 1 : 0);
                mpb.SetInteger(NewGaussianSplatRenderer.Props.DisplayChunks, gs.m_RenderMode == NewGaussianSplatRenderer.RenderMode.DebugChunkBounds ? 1 : 0);
